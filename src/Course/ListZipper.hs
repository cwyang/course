...
findLeft f (ListZipper l x r) =
  case break f l of
    (_, Nil) -> IsNotZ
    (n, ph:.pt) -> IsZ (ListZipper pt ph (reverse n ++ x:.r))
...
findRight f (ListZipper l x r) =
  case break f r of
    (_, Nil) -> IsNotZ
    (n, ph:.pt) -> IsZ (ListZipper (reverse n ++ x :. l) ph pt)
...
swapLeft ::
  ListZipper a
  -> MaybeListZipper a
swapLeft z | not (hasLeft z) = IsNotZ
...
swapRight ::
  ListZipper a
  -> MaybeListZipper a
swapRight z | not (hasRight z) = IsNotZ
...
moveLeftN' a b =
  ...
  where
    go acc 0 z = Right z
    go acc n z = case moveLeft z of
      IsNotZ -> Left acc
      IsZ z' -> go (succ acc) (pred n) z'
...
moveRightN' a b =
  ...
  where
    go acc 0 z = Right z
    go acc n z = case moveRight z of
      IsNotZ -> Left acc
      IsZ z' -> go (succ acc) (pred n) z'
...
nth ::
  ...
  -> MaybeListZipper a
nth n z =
  let z' = fromList $ toList z
  in case z' of
    IsNotZ -> IsNotZ
    IsZ z'' -> moveRightN n z''
...
deletePullLeft ::
  ListZipper a
  -> MaybeListZipper a
deletePullLeft (ListZipper Nil x r) =
  IsNotZ
deletePullLeft (ListZipper (l:.ls) x r) =
...
deletePullRight ::
  ListZipper a
  -> MaybeListZipper a
deletePullRight (ListZipper l x Nil) = IsNotZ
deletePullRight (ListZipper l x (r:.rs)) =
...
-- /Tip:/ Use `zipWith`
  ListZipper fl fx fr <*> ListZipper gl gx gr =
    ListZipper (zipWith ($) fl gl) (fx gx) (zipWith ($) fr gr)
...
instance Extend ListZipper where
  ...
  f <<= z = f <$> ListZipper (go moveLeft) z (go moveRight)
    where go mf = unfoldr (\b -> case b of
                              IsNotZ -> Empty
                              IsZ b -> Full (b, mf b)) (mf z)
{-
instance Extend ListZipper where
