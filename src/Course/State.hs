...
listWithState op tst = flip eval S.empty . op (State . lift2 (lift2 (,)) tst S.insert)
distinct ::
  Ord a =>
  List a
  -> List a
...
isHappy = contains 1 . firstRepeat . produce next
  where next = toInteger . sum . map (join (*) . digitToInt) . show'
